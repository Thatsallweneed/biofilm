    ...
    try:
        ...
        for image in uploaded_images:
            ...
            if os.path.splitext(filename)[1] == '.czi':
                czi_data_ = io.BytesIO(image.read())
                czi_data = czifile.imread(czi_data_)
            ...
            if czi_:
                with tempfile.NamedTemporaryFile(suffix=".czi", delete=False) as temp_file:
                    temp_file.write(czi_data_.getvalue())
                    temp_file.flush()
...
def search_attributes():
    try:
        ...
        searchOrganism = request.args.get('searchOrganism', '').strip()
        searchNcbi = request.args.get('searchNcbi', '').strip()
        print('searchOrganism',searchOrganism)
        print('searchNcbi',searchNcbi)

        if searchOrganism and searchNcbi:
            return jsonify({'error': 'Please provide either an organism name or an NCBI ID, not both.'}), 400
        elif not searchOrganism and not searchNcbi:
            return jsonify({'error': 'Please provide either an organism name or an NCBI ID.'}), 400

        connection = db_conn()
        with connection.cursor() as cur:
            if searchNcbi:
                ncbi_number = re.findall(r'\d+',searchNcbi)
                ncbi_id = config['ncbi_prefix']+ ncbi_number[0]
                cur.execute(config['organism_ncbiid_query'], ('%' + ncbi_id + '%',))
                organisms = cur.fetchall()
                # organisms = [{}]
                # organisms[0]['ncbi_id'] = ncbi_id
            else:
...
@app.route('/search_recommendation', methods=['GET'])
def search_recomendations():
    try:
        ...
        if search_type not in type_to_index_field:
            return jsonify([])

        index_name, field_name = type_to_index_field[search_type]
        ...
        suggestions = [hit["_source"][field_name] for hit in response['hits']['hits']]
        ...
        logger.error(f"While processing in Function {search_recomendations.__qualname__}, we got {sys.exc_info()[0]} Exception. \n '{e}' in Line Number {sys.exc_info()[2].tb_lineno}  File Name {os.path.basename(sys.exc_info()[2].tb_frame.f_code.co_filename)}")
        return jsonify({'error': f"{e}"})
